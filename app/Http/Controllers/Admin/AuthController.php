...
class AuthController extends JoshController
{
    ...
    public function postSignin(Request $request)
    {
        ...
        try {
            ...
            if ($user = Sentinel::authenticate($request->only(['email', 'password']), $request->get('remember-me', 0))) {
                ...
                $current_user_adminSide = User_admin::whereEmail($request->get('email'))->first();
    ...
    public function postSignup(UserRequest $request)
    {
        ...
        try {
            ...
            $user_admin -> first_name = $request->get('first_name');
            $user_admin -> last_name = $request->get('last_name');
            $user_admin -> email = $request->get('email');
            $user_admin -> password = bcrypt($request->get('password'));
            ...
            $activity_admin = new Activity_admin;
            $activity_admin -> description = 'Registered';
            $activity_admin -> user_id = $current_user_adminSide->id;
            $activity_admin -> ip_address = $request->getClientIp();
            $activity_admin -> save();
    ...
    public function getLogout(Request $request)
    {
        ...
        $current_user_adminSide = User_admin::whereEmail($user['email'])->first();
        $activity_admin = new Activity_admin;
        $activity_admin -> description = 'LoggedOut';
        $activity_admin -> user_id = $current_user_adminSide->id;
        $activity_admin -> ip_address = $request->getClientIp();
        $activity_admin -> save();
